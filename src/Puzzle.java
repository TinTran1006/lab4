...
public class Puzzle {
	...
	public int computeH1(Node currentState) {
		int output = 0;
		this.state = new int[currentState.state.length][currentState.state[0].length];
		/* Enter your code here */
		int[][] array = null;
		for (int row = 0; row < state.length; row++) {
			for (int col = 0; col < state.length; col++) {
				if (array[row][col] != currentState.state[row][col])
					output++;
			}
		}
	...
	public int computeH2(Node currentState) {
		int result = 0;
		for (int tile = 1; tile < MAX_COL; tile++) {
			int [] currentPos = currentState.getLocation(tile);
			int [] currentGoal = goalState.getLocation(tile);
